// Copyright (c) The University of Dundee 2018-2019
// This file is part of the Research Data Management Platform (RDMP).
// RDMP is free software: you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation, either version 3 of the License, or (at your option) any later version.
// RDMP is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more details.
// You should have received a copy of the GNU General Public License along with RDMP. If not, see <https://www.gnu.org/licenses/>.

using NUnit.Framework;
using Rdmp.Core.CommandExecution;
using Rdmp.Core.CommandExecution.AtomicCommands;
using Rdmp.Core.Repositories;
using System;
using System.Collections.Generic;
using System.Linq;
using Tests.Common;

namespace Rdmp.Core.Tests.CommandExecution;

public class TestCommandsAreSupported : UnitTests
{
    private CommandInvoker invoker;

    // Commands known to not be CLI compatible
    private static readonly HashSet<Type> KnownIncompatibleCommands = new()
    {
        typeof(ExecuteCommandSetDataAccessContextForCredentials), // Not currently CLI compatible
    };

    [OneTimeSetUp]
    public void Init()
    {
        var activator = GetActivator();
        invoker = new CommandInvoker(activator);
    }

    /// <summary>
    /// Auto-discovers all IAtomicCommand types and generates test cases dynamically.
    /// This eliminates the need for manual [TestCase] attributes.
    /// </summary>
    public static IEnumerable<Type> GetAllCommandTypes()
    {
        return MEF.GetTypes<IAtomicCommand>()
            .Where(t => !KnownIncompatibleCommands.Contains(t))
            .OrderBy(t => t.FullName);
    }

    [Test]
    [TestCaseSource(nameof(GetAllCommandTypes))]
    public void TestIsSupported(Type t)
    {
        Assert.That(invoker.WhyCommandNotSupported(t), Is.Null, $"Type {t} was not supported by CommandInvoker");
    }
}