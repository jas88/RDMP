...
BEGIN
	...
	SET @sql = 'SELECT DISTINCT ' + QUOTENAME(@identifiableColumn) + ',' + QUOTENAME(@anonymousColumn) + ' FROM ' + QUOTENAME(@tableName) + ' INNER JOIN #tempBatch b on ' + QUOTENAME(@identifiableColumn) + '= b.Identifier'
