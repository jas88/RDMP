...
-- Create CacheProgress table if it doesn't exist
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME='CacheProgress')
BEGIN
...
-- ADD NEW FOREIGN KEY COLUMN to LoadSchedule (if table still exists)
...
BEGIN
	...
	(LoadSchedule_ID, CacheFillProgress, CacheLagPeriod, AlternativeCacheLocation)
	SELECT ID, CacheProgress, CacheLagPeriod, AlternativeCacheLocation FROM [LoadSchedule]')
